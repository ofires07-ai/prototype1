...
public class IS_Enemy : MonoBehaviour
{
    ...
    // 공격 애니메이션 쿨타임 부여(보스 공격 이상해서 진행)
    [Tooltip("✅ 공격 애니메이션 클립의 정확한 이름 (예: 'Enemy_Attack')")] // [✨ 추가], IS 수정
    //[SerializeField] private string attackClipName = "Attack_Clip"; 
    private float attackCooldown; // [✨ 변경] 자동 계산된 쿨타임 저장
    ...
    void Start()
    {
        ...
        {
            // 애니메이션 길이 + 0.1초 여유 시간
            attackCooldown = duration + 0.1f;
            Debug.Log($"[AI] {name}: 공격 애니메이션 길이를 {duration:F2}초로 감지하여 쿨타임을 {attackCooldown:F2}초로 설정함.");
        }
        else
        {
            // 못 찾았으면 기본값 1.5초 (혹은 안전장치)
            attackCooldown = 1.5f;
            Debug.LogWarning($"[AI] {name}: 'Attack'이 포함된 애니메이션을 못 찾았습니다. 기본 쿨타임(1.5s)을 사용합니다.");
        }
    ...
    /// <summary>
/// 애니메이터 안에서 "Attack"이라는 단어가 들어간 클립을 찾아 길이를 반환합니다.
/// </summary>
    ...
    // 모든 클립 가져오기
        ...
        {
            // 💡 핵심: 클립 이름에 "Attack" 또는 "attack"이 포함되어 있는지 확인
            if (clip.name.Contains("Attack") || clip.name.Contains("attack"))
            {
                // 찾았다! (예: "Goblin_Attack", "Boss_Attack_Up" 등 모두 통과)
                return clip.length;
            }
        }
    
        return 0f; // "Attack"이 들어간 클립이 없음
    }
    /// <summary>
    /// (전투) 타겟을 추격하거나 공격합니다.
    /// </summary>
    void HandleCombat(Transform target)
    {
            ...
            if (Time.time > lastAttackTime + attackCooldown)
        {
            ...
            // (1) 공격 트리거 발동 (FSM이 'Idle' -> 'Attack'으로 전환)
            animator.SetTrigger("Attack"); 
            
            // (2) 마지막 공격 시간 갱신 (지금 공격했음 기록)
            lastAttackTime = Time.time;
        }
        else
        {
            // 쿨타임 중: 공격 대기 (Speed=0 상태 유지)
            // 아무것도 하지 않음 (Attack 트리거 재호출 방지)
        }
